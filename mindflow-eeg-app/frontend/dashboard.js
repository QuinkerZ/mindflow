...
function initRadarChart() {
    ...
    bwRadarChart = new Chart(ctx.getContext('2d'), {
        type: 'radar',
        data: {
            labels: ['Stress', 'Fatigue', 'Low Energy', 'Brain Overload', 'Relaxation'],
            datasets: [{
                label: 'Current State',
                data: [0, 0, 0, 0, 0],
        ...
        if (total > 0) {
            ...
            if (data.params) {
                ...
                if (window.updateRadarChart) {
                    window.updateRadarChart([p.stress, p.fatigue, p.lowEnergy, p.brainOverload, p.relaxation]);
                }
            }  else {
                // Fallback: use band percentages as rough proxy (for demo purposes)
                if (window.updateRadarChart) {
                    window.updateRadarChart([deltaPct, thetaPct, alphaPct, betaPct, gammaPct]);
                }
            }
...
function highlightDominantParam(params) {
    ...
    const nameMap = { stress: 'STRESS', fatigue: 'FATIGUE', lowEnergy: 'LOW ENERGY', brainOverload: 'BRAIN OVERLOAD', relaxation: 'RELAXATION' };
...
function startSimulatedData() {
    ...
    simInterval = setInterval(() => {
        ...
        if (window.updateRadarChart) {
            window.updateRadarChart([simParams.stress, simParams.fatigue, simParams.lowEnergy, simParams.brainOverload, simParams.relaxation]);
        }
        ...
        if (diagnosticText) diagnosticText.innerHTML = `<strong>Simulated: ${simState}</strong> (demo)`;
...
function simulateMentalStateParams(bands) {
    ...
    function sig(x, c, s) { return 100 / (1 + Math.exp(-s * (x - c))); }
    ...
    if (rawTotal > 0) {
        stress = (stress/rawTotal)*100; fatigue = (fatigue/rawTotal)*100;
        lowEnergy = (lowEnergy/rawTotal)*100; brainOverload = (brainOverload/rawTotal)*100;
        relaxation = (relaxation/rawTotal)*100;
    }
    const params = { stress, fatigue, lowEnergy, brainOverload, relaxation };
    const dominant = Object.keys(params).reduce((a,b) => params[a] > params[b] ? a : b);
    return { stress: Math.round(stress), fatigue: Math.round(fatigue), lowEnergy: Math.round(lowEnergy), brainOverload: Math.round(brainOverload), relaxation: Math.round(relaxation), dominant };
}
...
function updateBrainStateParamsFromServer(params) {
    ...
    const nameMap = { stress: 'Stress', fatigue: 'Fatigue', lowEnergy: 'Low Energy', brainOverload: 'Brain Overload', relaxation: 'Relaxation' };
    if (detectedStateParam) detectedStateParam.innerText = nameMap[params.dominant] || 'Analyzing…';
}
...
const PARAM_REMEDY_MAP = {
    stress:        { label: 'Stress',         remedy: 'Audio Therapy',        page: 'solutions.html?condition=stress' },
    fatigue:       { label: 'Fatigue',        remedy: 'Visual Therapy',       page: 'solutions.html?condition=fatigue' },
    lowEnergy:     { label: 'Low Energy',     remedy: 'Physical Activity',    page: 'solutions.html?condition=low-energy' },
    brainOverload: { label: 'Brain Overload', remedy: 'Visual Therapy',       page: 'solutions.html?condition=brain-overload' },
    relaxation:    { label: 'Relaxation',     remedy: 'Lifestyle Suggestions', page: 'solutions.html?condition=relaxation' }
};
...
function performAnalysis(duration) {
    ...
    setTimeout(() => {
        ...
        if (bandHistory.length === 0) {
            showResult('Relaxation', duration, { stress: 10, fatigue: 10, lowEnergy: 15, brainOverload: 5, relaxation: 60, dominant: 'relaxation' });
            return;
        }
...
function showResult(condition, duration, params) {
        ...
        paramBreakdown = `
            <div style="margin: 1rem 0; text-align: left;">
                ...
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    ${Object.entries(PARAM_REMEDY_MAP).map(([key, val]) => {
    ...
    const dominantKey = params?.dominant || 'relaxation';
    const info = PARAM_REMEDY_MAP[dominantKey];
    if (diagnosticText) diagnosticText.innerHTML = `<strong>Detected: ${condition}</strong> · ${info?.remedy || 'Lifestyle Suggestions'} recommended · based on ${duration || 30}s analysis.`;
    if (diagnosticSolutionsBtn) {
        diagnosticSolutionsBtn.style.display = 'inline-block';
        diagnosticSolutionsBtn.onclick = () => window.location.href = info?.page || 'solutions.html?condition=stress';
    }
...
function getParamSolution(paramKey) {
    const map = {
        stress:        'Your brain is showing elevated stress. Try Audio Therapy — calming music, binaural beats, or guided breathing exercises to reduce cortisol levels.',
        fatigue:       'Your brain is tired. Try Visual Therapy — nature scenes, guided imagery, or relaxing visual patterns to restore mental energy.',
        lowEnergy:     'Your energy is low. Try Physical Activity — light stretching, a short walk, or neck/shoulder exercises to boost alertness.',
        brainOverload: 'Your brain is overloaded with activity. Try Visual Therapy — slow-moving visuals, mandala coloring, or eye-rest exercises to calm neural activity.',
        relaxation:    'You are in a relaxed state! Lifestyle Suggestions — maintain this with good sleep, hydration, and mindfulness practices.'
    };
